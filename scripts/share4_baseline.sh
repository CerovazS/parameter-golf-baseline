...
#SBATCH --job-name=pgolf-share4-baseline
...
export RUN_ID="share4_baseline_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
...
srun uv run bash ./tools/launch_run.sh \
    ...
    --gpus 4 \
    --run-id "$RUN_ID" \
    --hypothesis "4xA100 700s | Baseline no sharing — confronto diretto serie share4"
