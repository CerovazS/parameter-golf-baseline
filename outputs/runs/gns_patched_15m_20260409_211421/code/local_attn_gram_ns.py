...
class Hyperparameters:
    ...
    local_attn_window = int(os.environ.get("LOCAL_ATTN_WINDOW", 512))  # sliding window size for local attention layers
    global_attn_layers = os.environ.get("GLOBAL_ATTN_LAYERS", "5,10")  # comma-sep layer indices that keep global attention
    # GPTQ calibration
    gptq_calib_batches = int(os.environ.get("GPTQ_CALIB_BATCHES", 256))
    gptq_block_size = int(os.environ.get("GPTQ_BLOCK_SIZE", 128))

# --- Gram Newton-Schulz orthogonalization (Dao-AILab) ---
# Replaces the standard Newton-Schulz iteration with the Gram variant:
# - Iterates on Gram matrix R = XX^T (symmetric, smaller) instead of X directly
# - Uses float16 (not bfloat16) for higher mantissa precision near 1.0
# - Time-varying YOU coefficients instead of fixed (3.4445, -4.7750, 2.0315)
# - Restart after iteration 2 to prevent fp16 eigenvalue divergence
# - ~42% fewer FLOPs at aspect ratio 4, CuTe-DSL symmetric GEMM kernels on sm_90+
...
class Muon(torch.optim.Optimizer):
    ...
    def _build(self):
        ...
        # Gram Newton-Schulz instance (compiled internally, supports batched 3D inputs)
        self._gram_ns = GramNewtonSchulz(
            ns_coefficients=YOU_COEFFICIENTS,
            ns_use_kernels=True,
            gram_newton_schulz_reset_iterations=[2],
            compile_kwargs=None,  # E03 patch: reduce-overhead CUDA graphs conflict with model-level torch.compile
        )
        self._built = True
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                update = self._gram_ns(update).contiguous()
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.window_size = (-1, -1)  # (-1,-1)=global, (W-1,0)=causal sliding window of size W; set by GPT.__init__
...
class GPT(nn.Module):
    def __init__(
        ...
        # Local (sliding window) attention: all layers get local by default, global_attn_layers stay global
        if local_attn_window > 0:
            global_set = set(int(x) for x in global_attn_layers.split(",") if x.strip()) if global_attn_layers else set()
            for i in range(num_layers):
                if i not in global_set:
                    self.blocks[i].attn.window_size = (local_attn_window - 1, 0)
        self._init_weights()
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
                 ...
                 local_attn_window=0, global_attn_layers=""):
        super().__init__()
        self.tie_embeddings = tie_embeddings
        self.logit_softcap = logit_softcap
        self.num_layers = num_layers
...
def main() -> None:
    ...
    args = Hyperparameters()
    # Gram Newton-Schulz uses torch.compile internally; increase cache for distinct shapes
    ...
    base_model = GPT(
        ...
        global_attn_layers=args.global_attn_layers,
    ).to(device).bfloat16()
    ...
    hessian_model = _HessianGPT(
        ...
        local_attn_window=args.local_attn_window, global_attn_layers=args.global_attn_layers,
    ).to(device).bfloat16()
