...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 10, eps: float = 1e-7) -> Tensor:
    ...
    for _ in range(steps):
        A = X @ X.T
        ba_plus_cAA(A, alpha=c, beta=b, out=B)  # B = c*A@A.T + b*A (Triton fused)
        X = a * X + B @ X
...
def main() -> None:
    ...
    args = Hyperparameters()
    # Note: torch.compile removed — zeropower_via_newtonschulz5 uses the Triton ba_plus_cAA kernel
    # which has a symmetric double-write pattern incompatible with TorchDynamo tracing.
