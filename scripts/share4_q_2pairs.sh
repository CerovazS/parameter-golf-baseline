...
#SBATCH --job-name=pgolf-share4-q-2pairs
...
export RUN_ID="share4_q_2pairs_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
...
srun uv run bash ./tools/launch_run.sh \
    ...
    --gpus 4 \
    --run-id "$RUN_ID" \
    --hypothesis "4xA100 700s | Q rq=256 pairs=2 — extend Q sharing"
