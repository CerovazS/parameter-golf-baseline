...
#SBATCH --job-name=pgolf-share4-o-q-2p
...
export SHARING_NUM_PAIRS=2
export RUN_ID="share4_o_q_2p_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
...
srun uv run bash ./tools/launch_run.sh \
    ...
    --gpus 4 \
    --run-id "$RUN_ID" \
    --hypothesis "4xA100 700s | O+Q ro=rq=256 pairs=2 — combined O and Q sharing extended"
