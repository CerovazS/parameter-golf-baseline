...
#SBATCH --job-name=pgolf-share-v-o-low
...
# ── Experiment config ──────────────────────────────────────────────────────
export SHARING_RANK_V=64
export SHARING_RANK_O=128
export MAX_WALLCLOCK_SECONDS=500
export VAL_LOSS_EVERY=200
export HYPOTHESIS="SharedC V+O rank_v=64 rank_o=128: test compressione aggressiva"
export RUN_ID="share_v_o_low_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
