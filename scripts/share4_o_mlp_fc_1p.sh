...
#SBATCH --job-name=pgolf-share4-o-mlpfc-1p
...
export RUN_ID="share4_o_mlp_fc_1p_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
...
srun uv run bash ./tools/launch_run.sh \
    ...
    --gpus 4 \
    --run-id "$RUN_ID" \
    --hypothesis "4xA100 700s | O+MLP_FC ro=r_fc=256 pairs=1 — combined O and MLP fc sharing"
