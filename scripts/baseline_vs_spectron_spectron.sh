...
#SBATCH --job-name=pgolf-spectron-4xA100
...
# MATRIX_LR and RANK_RATIO are safe with :- because .env does not set them.
export MATRIX_LR="${MATRIX_LR:-0.04}"
export RANK_RATIO="${RANK_RATIO:-0.25}"
export MLP_MULT="${MLP_MULT:-2}"
LR_TAG="${MATRIX_LR//./}"
MLP_TAG=""
[[ "$MLP_MULT" != "2" ]] && MLP_TAG="_mlp${MLP_MULT}"
export RUN_ID="spectron_lr${LR_TAG}${MLP_TAG}_4xA100_12min_$(date +%Y%m%d)_job${SLURM_JOB_ID}"
export MAX_WALLCLOCK_SECONDS=720
export VAL_LOSS_EVERY=200
export HYPOTHESIS="Spectron low-rank (rank_ratio=${RANK_RATIO}, lr=${MATRIX_LR}, mlp_mult=${MLP_MULT}) on 4xA100, 12-min wallclock"
