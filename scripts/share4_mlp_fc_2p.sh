...
#SBATCH --job-name=pgolf-share4-mlp-fc-2p
...
export RUN_ID="share4_mlp_fc_2p_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
...
srun uv run bash ./tools/launch_run.sh \
    ...
    --gpus 4 \
    --run-id "$RUN_ID" \
    --hypothesis "4xA100 700s | MLP_FC rank=256 pairs=2 — extend MLP fc sharing"
