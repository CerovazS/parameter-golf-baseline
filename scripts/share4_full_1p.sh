...
#SBATCH --job-name=pgolf-share4-full-1p
...
export RUN_ID="share4_full_1p_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
...
srun uv run bash ./tools/launch_run.sh \
    ...
    --gpus 4 \
    --run-id "$RUN_ID" \
    --hypothesis "4xA100 700s | O+Q+MLP_FC+MLP_PROJ rank=256 pairs=1 — full single-pair sharing upper bound"
