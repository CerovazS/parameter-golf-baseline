...
#SBATCH --job-name=pgolf-share4-o-4pairs
...
export RUN_ID="share4_o_4pairs_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
...
srun uv run bash ./tools/launch_run.sh \
    ...
    --gpus 4 \
    --run-id "$RUN_ID" \
    --hypothesis "4xA100 700s | O ro=256 pairs=4 — extend O sharing"
