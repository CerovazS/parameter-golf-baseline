...
#SBATCH --job-name=pgolf-share4-v-o
...
export RUN_ID="share4_v_o_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
...
srun uv run bash ./tools/launch_run.sh \
    ...
    --gpus 4 \
    --run-id "$RUN_ID" \
    --hypothesis "4xA100 700s | SharedC V+O rank_v=128 rank_o=256 pair=(0,4)"
