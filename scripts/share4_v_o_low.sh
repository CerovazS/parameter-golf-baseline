...
#SBATCH --job-name=pgolf-share4-v-o-low
...
export RUN_ID="share4_v_o_low_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
...
srun uv run bash ./tools/launch_run.sh \
    ...
    --gpus 4 \
    --run-id "$RUN_ID" \
    --hypothesis "4xA100 700s | SharedC V+O rank_v=64 rank_o=128 pair=(0,4)"
