...
#SBATCH --job-name=pgolf-share4-solo-v
...
export RUN_ID="share4_solo_v_$(date +%Y%m%d)_job${SLURM_JOB_ID:-local}"
...
srun uv run bash ./tools/launch_run.sh \
    ...
    --gpus 4 \
    --run-id "$RUN_ID" \
    --hypothesis "4xA100 700s | SharedC solo V rank_v=128 pair=(0,4)"
